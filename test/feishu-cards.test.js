...
function collectButtons(card) {
  const elements = (card.body && card.body.elements) || card.elements || [];
  const buttons = [];
  for (const el of elements) {
    if (el.tag === 'button' && el.text) {
      buttons.push(el);
    } else if (el.tag === 'action' || el.tag === 'form') {
      for (const sub of el.actions || el.elements || []) {
        if (sub.tag === 'button') buttons.push(sub);
      }
    } else if (el.tag === 'column_set') {
      for (const col of el.columns || []) {
        for (const sub of col.elements || []) {
          if (sub.tag === 'button') buttons.push(sub);
        }
      }
    }
...
function buttonValue(btn) {
  return btn.value || ((btn.behaviors || []).find((b) => b.type === 'callback') || {}).value;
}

function collectModelButtons(card) {
  return collectButtons(card).filter((button) => {
    const v = buttonValue(button);
    return v && v.action && v.action.startsWith('cmd:/model ') && !v.action.startsWith('cmd:/model --page ');
  });
}
...
function collectNavigationButtons(card) {
  return collectButtons(card).filter((button) => {
    const v = buttonValue(button);
    return v && v.action && v.action.startsWith('cmd:/model --page ');
  });
}
...
test('renderSessionListCard 空列表时显示提示', () => {
  ...
  assert.ok(card.body.elements);
  assert.ok(card.body.elements.some((el) => el.tag === 'markdown'));
});
...
test('renderSessionListCard 包含 session 状态和按钮', () => {
  ...
  assert.ok(card.header);
  assert.ok(card.body.elements.length >= 2);
  const buttons = collectButtons(card);
  assert.ok(buttons.length > 0, 'card should have buttons');
});
...
test('renderSessionListCard 当前 session 有标记', () => {
  ...
  const card = renderSessionListCard(sessions, 'wks_abc1');
  const mdEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content && el.content.includes('wks_abc1'));
  assert.ok(mdEl);
  assert.ok(mdEl.content.includes('wks_abc1'));
  assert.ok(mdEl.content.includes('idle'));
  assert.ok(mdEl.content.includes('当前绑定'));
});
...
test('非焦点 session 有设为焦点按钮', () => {
  ...
  const focusBtn = collectButtons(card).find((b) => b.text && b.text.content === '设为焦点');
  assert.ok(focusBtn, '非焦点 session 应有"设为焦点"按钮');
  assert.deepEqual(buttonValue(focusBtn), { action: 'cmd:/use wks_other' });
...
test('焦点 session 标记已聚焦', () => {
  ...
  const card = renderSessionListCard(sessions, 'wks_focus');
  const focusBtn = collectButtons(card).find((b) => b.text && b.text.content === '已聚焦');
  assert.ok(focusBtn, '焦点 session 应有"已聚焦"标记');
  assert.equal(focusBtn.type, 'default');
  assert.deepEqual(buttonValue(focusBtn), { action: 'cmd:/use wks_focus' });
});
...
test('设为焦点按钮携带 routeKey', () => {
  ...
  const focusBtn = collectButtons(card).find((b) => b.text && b.text.content === '设为焦点');
  assert.deepEqual(buttonValue(focusBtn), { action: 'cmd:/use wks_other', routeKey: 'feishu:oc_chat1:root:om_root1' });
});
...
test('renderSessionListCard running 状态显示蓝色', () => {
  ...
  const mdEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content && el.content.includes('running'));
  assert.ok(mdEl.content.includes('running'));
});
...
test('renderUnboundRouteCard 提供 attach/new/list 按钮', () => {
  ...
  assert.ok(card.header);
  assert.ok(card.body.elements.some((el) => el.tag === 'markdown'));
  const actions = collectButtons(card).map((b) => buttonValue(b).action);
...
test('renderModelListCard 对完整稳定去重序列分页为 20、20、13', () => {
  ...
  const actions = pageButtons.flat().map((button) => buttonValue(button).action);
  ...
  assert.deepEqual(cards.map((card) => card.body.elements.find((el) => el.tag === 'markdown' && el.content.startsWith('第 ')).content), [
    '第 1 / 3 页',
    '第 2 / 3 页',
    '第 3 / 3 页',
  ]);
  assert.ok(pageButtons.flat().every((button) => buttonValue(button).routeKey === routeKey));
...
test('renderModelListCard 导航遵守页面边界并透传 routeKey', () => {
  ...
  assert.deepEqual(collectNavigationButtons(cards[0]).map((button) => buttonValue(button)), [
    { action: 'cmd:/model --page 2', routeKey },
  ]);
  assert.deepEqual(collectNavigationButtons(cards[1]).map((button) => buttonValue(button)), [
    { action: 'cmd:/model --page 1', routeKey },
    { action: 'cmd:/model --page 3', routeKey },
  ]);
  assert.deepEqual(collectNavigationButtons(cards[2]).map((button) => buttonValue(button)), [
    { action: 'cmd:/model --page 2', routeKey },
  ]);
...
test('renderModelListCard 将无效页码归一化到有效的 1-based 页码', () => {
  ...
  for (const item of cases) {
    ...
    const pageText = card.body.elements.find((el) => el.tag === 'markdown' && el.content.startsWith('第 '));
    assert.equal(pageText.content, item.expected);
  }
});
...
test('renderModelListCard 对其余模型保持 provider 首次出现顺序和组内顺序', () => {
  ...
  assert.deepEqual(collectModelButtons(card).map((button) => buttonValue(button).action), [
...
test('renderModelListCard 保持空列表与单页卡片兼容', () => {
  ...
  assert.ok(emptyCard.body.elements.some((el) => el.tag === 'markdown' && el.content === '暂无可用模型。'));
  ...
  assert.equal(singleCard.body.elements.find((el) => el.tag === 'markdown' && el.content.startsWith('第 ')).content, '第 1 / 1 页');
...
test('renderHelpCard 基于命令元数据生成帮助按钮', () => {
  ...
  const text = card.body.elements
    .filter((el) => el.tag === 'markdown' && el.content)
    .map((el) => el.content)
    .join('\n');
  const actions = collectButtons(card).map((button) => buttonValue(button));
...
test('renderErrorCard 显示错误信息', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('session not found'));
});
...
test('buildPermissionCard body 包含权限标题', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('执行 rm 命令'));
});
...
test('buildPermissionCard 包含允许、始终允许和拒绝按钮', () => {
  ...
  const buttons = collectButtons(card);
  assert.ok(buttons.some((b) => b.text && b.text.content === '允许' && b.type === 'primary'));
  assert.ok(buttons.some((b) => b.text && b.text.content === '始终允许' && b.type === 'default'));
  assert.ok(buttons.some((b) => b.text && b.text.content === '拒绝' && b.type === 'danger'));
});
...
test('buildPermissionCard 按钮 value 携带 permit 命令', () => {
  ...
  const buttons = collectButtons(card);
  const allowBtn = buttons.find((b) => b.text && b.text.content === '允许');
  assert.match(buttonValue(allowBtn).action, /cmd:\/permit perm_abc allow/);
  assert.match(buttonValue(allowBtn).action, /wks_s1/);
  assert.equal(buttonValue(allowBtn).routeKey, 'rk1');
  const alwaysBtn = buttons.find((b) => b.text && b.text.content === '始终允许');
  assert.match(buttonValue(alwaysBtn).action, /cmd:\/permit perm_abc always/);
  const denyBtn = buttons.find((b) => b.text && b.text.content === '拒绝');
  assert.match(buttonValue(denyBtn).action, /cmd:\/permit perm_abc deny/);
});
...
test('buildPermissionCard 缺少 title 时显示未知权限请求', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('未知权限请求'));
});
...
test('buildPermissionCard metadata 显示命令信息', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('npm test'));
});
...
test('buildPermissionCard metadata 显示权限匹配规则', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('external\\_directory'));
  assert.ok(textEl.content.includes('匹配规则'));
  assert.ok(textEl.content.includes('sacpServ'));
});
...
test('buildPermissionRepliedCard allow 显示绿色模板和最终选择', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('最终选择'));
  assert.ok(textEl.content.includes('已允许'));
  assert.ok(textEl.content.includes('perm\\_abc'));
});
...
test('buildPermissionRepliedCard deny 显示灰色模板和最终选择', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('最终选择'));
  assert.ok(textEl.content.includes('已拒绝'));
  assert.ok(textEl.content.includes('perm\\_abc'));
});
...
test('buildPermissionRepliedCard always 显示绿色模板和始终允许', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('最终选择'));
  assert.ok(textEl.content.includes('已始终允许'));
  assert.ok(textEl.content.includes('perm\\_abc'));
});
...
test('buildQuestionCard confirm 仅生成 preview:/answer 预览动作', () => {
  ...
  assert.match(buttonValue(allowBtn).action, /preview:\/answer q_1 allow/);
  assert.match(buttonValue(denyBtn).action, /preview:\/answer q_1 deny/);
  assert.ok(buttonValue(allowBtn).action.includes('wks_s1'));
});
...
test('buildQuestionCard single_select 渲染 N 个 button，value 含 option.value', () => {
  ...
  assert.match(buttonValue(buttons[0]).action, /preview:\/answer q_2 gpt4/);
  assert.match(buttonValue(buttons[1]).action, /preview:\/answer q_2 claude/);
});
...
test('buildQuestionCard multi_select 渲染 multi_select_static + 提交 button，name=question_answer', () => {
  ...
  const formEl = card.body.elements.find((el) => el.tag === 'form');
  assert.ok(formEl);
  const selectEl = formEl.elements.find((a) => a.tag === 'multi_select_static');
  assert.ok(selectEl);
  ...
  const submitBtn = formEl.elements.find((a) => a.tag === 'button');
  assert.ok(submitBtn);
  assert.match(buttonValue(submitBtn).action, /preview:\/answer q_3 --form/);
});
...
test('buildQuestionCard text 渲染 input + 提交 button，name=question_answer', () => {
  ...
  const formEl = card.body.elements.find((el) => el.tag === 'form');
  assert.ok(formEl);
  const inputEl = formEl.elements.find((a) => a.tag === 'input');
  assert.ok(inputEl);
  assert.equal(inputEl.name, 'question_answer');
  assert.equal(inputEl.placeholder.content, '请描述你的需求');
  const submitBtn = formEl.elements.find((a) => a.tag === 'button');
  assert.ok(submitBtn);
  assert.match(buttonValue(submitBtn).action, /preview:\/answer q_4 --form/);
});
...
test('buildQuestionCard 未知 inputMode 降级为 confirm', () => {
  ...
  assert.ok(allowBtn);
  assert.ok(denyBtn);
  assert.match(buttonValue(allowBtn).action, /preview:\/answer q_5 allow/);
});
...
test('buildQuestionCard select 缺少 options 渲染错误状态', () => {
  ...
  const textEl = card.body.elements.find((el) => el.tag === 'markdown' && el.content);
  assert.ok(textEl.content.includes('选项缺失'));
  const formEl = card.body.elements.find((el) => el.tag === 'action');
  assert.equal(formEl, undefined);
});
...
test('buildQuestionCard routeKey 透传到 buildButtonValue', () => {
  ...
  const buttons = collectButtons(card);
  const allowBtn = buttons.find((b) => b.text && b.text.content === '允许');
  assert.equal(buttonValue(allowBtn).routeKey, 'rk_test');
});
...
test('buildNativeQuestionStatusCard 覆盖处理、终态、降级和过期反馈', () => {
  ...
  for (const [status, message] of Object.entries(expected)) {
    ...
    const content = card.body ? card.body.elements[0].content : card.body.elements.find((element) => element.tag === 'markdown').text.content;
