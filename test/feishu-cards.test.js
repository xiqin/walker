...
test('buildNativeQuestionCard 多选输出完整 Card JSON 2.0 表单一次提交', () => {
  const card = buildNativeQuestionCard({
    requestID: 'req_multi',
    questionIndex: 0,
    questionCount: 1,
  ...
  assert.match(markdown.content, /\*\*1\. 认证\*\*/);
  assert.match(markdown.content, /登录鉴权/);
  assert.match(markdown.content, /\*\*2\. 通知\*\*/);
  assert.match(markdown.content, /消息推送/);
  const form = card.body.elements.find((element) => element.tag === 'form');
  assert.ok(form, '多选卡片必须包含 v2 form');
  const checkbox = form.elements.find((el) => el.tag === 'multi_select_static');
  assert.ok(checkbox, 'v2 form 必须包含 multi_select_static');
  assert.equal(checkbox.name, 'question_selected');
  assert.equal(checkbox.width, 'fill');
  assert.equal(checkbox.required, true);
  assert.equal(checkbox.placeholder.content, '请选择一个或多个选项');
  assert.equal(checkbox.options.length, 2);
  assert.equal(checkbox.options[0].text.content, '认证');
  assert.equal(checkbox.options[0].value, 'option_0');
  assert.equal(checkbox.options[1].text.content, '通知');
  assert.equal(checkbox.options[1].value, 'option_1');
  ...
  const toggleButtons = form.elements.filter((el) => el.tag === 'button' && (el.value && el.value.action && el.value.action.includes('--toggle')));
  assert.equal(toggleButtons.length, 0, '新多选卡片不得再生成 --toggle 按钮');
  const submitActionButtons = form.elements.filter((el) => el.tag === 'button' && (el.value && el.value.action && el.value.action.includes('--submit')));
  assert.equal(submitActionButtons.length, 0, '新多选卡片不得再依赖旧 --submit 按钮');
  assert.equal(card.elements, undefined);
