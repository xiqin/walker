...
const CONFIG_DEFINITIONS = Object.freeze([
  ...
  { env: 'CLAUDE_PERMISSION_MODE', label: '权限模式', group: 'claude', defaultValue: '', type: 'claude-permission-mode', input: { type: 'enum', values: ['', 'acceptEdits', 'auto', 'manual', 'dontAsk', 'plan', 'bypassPermissions'], labels: { '': '未指定（省略 --permission-mode）', bypassPermissions: 'bypassPermissions（需另启危险确认）' } }, description: '留空会省略 Claude permission flag；旧 default 输入仅作为兼容迁移，不作为 Claude 原生枚举传递' },
  ...
  { env: 'CLAUDE_CONFIG_DIR', label: 'Transcript 根目录', group: 'claude', defaultValue: '', type: 'string', input: { type: 'text' }, description: '仅用于 Walker 读取 Claude transcript，不作为 Claude 设置目录传递' },
  { env: 'CLAUDE_TOOLS', label: 'Claude 原生 tools', group: 'claude', defaultValue: '', type: 'list', input: { type: 'list', itemType: 'string' }, description: 'Claude 原生工具列表，不等同于 OpenCode allow/ask/deny 规则' },
  { env: 'CLAUDE_AGENTS', label: 'Claude agents', group: 'claude', defaultValue: '', type: 'json-object', input: { type: 'json-object' }, description: 'Claude 原生 --agents JSON object，例如 {"reviewer":{"model":"opus"}}' },
  { env: 'CLAUDE_MCP_CONFIGS', label: 'MCP 配置文件', group: 'claude', defaultValue: '', type: 'json-list', input: { type: 'json-list', itemType: 'string' } },
  { env: 'CLAUDE_STRICT_MCP_CONFIG', label: '严格 MCP 配置', group: 'claude', defaultValue: 'false', type: 'boolean', input: { type: 'boolean', values: ['true', 'false'] } },
  { env: 'CLAUDE_SETTINGS_FILE', label: 'Settings 文件', group: 'claude', defaultValue: '', type: 'string', input: { type: 'text' } },
  { env: 'CLAUDE_SETTING_SOURCES', label: 'Settings 来源', group: 'claude', defaultValue: '', type: 'enum-list', input: { type: 'list', values: ['user', 'project', 'local'], itemType: 'enum' } },
  { env: 'CLAUDE_PLUGIN_DIRS', label: 'Plugin 目录', group: 'claude', defaultValue: '', type: 'list', input: { type: 'list', itemType: 'string' } },
  { env: 'CLAUDE_BARE', label: 'Bare 模式', group: 'claude', defaultValue: 'false', type: 'boolean', input: { type: 'boolean', values: ['true', 'false'] } },
  { env: 'CLAUDE_SAFE_MODE', label: 'Safe mode', group: 'claude', defaultValue: 'false', type: 'boolean', input: { type: 'boolean', values: ['true', 'false'] } },
  { env: 'CLAUDE_DISABLE_SLASH_COMMANDS', label: '禁用 slash commands', group: 'claude', defaultValue: 'false', type: 'boolean', input: { type: 'boolean', values: ['true', 'false'] } },
  { env: 'CLAUDE_ALLOW_BYPASS_PERMISSIONS', label: '允许 bypass permissions', group: 'claude', defaultValue: 'false', type: 'boolean', input: { type: 'boolean', values: ['true', 'false'] }, description: '默认禁止；启用后仅传递 Claude 原生 bypass，不代表 OpenCode 权限语义' },
